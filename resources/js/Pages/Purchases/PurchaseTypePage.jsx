...
const PurchaseTypePage = ({ purchases, purchaseType }) => {
        ...
        <AuthenticatedLayout
            header={
                <div className='flex items-center justify-between'>
                    <h2 className="text-lg leading-tight text-gray-800">{`${purchaseType} Purchases`}</h2>
                    <PrimaryIconLink href={route('purchases.create')} icon={faAdd}>Add Purchases</PrimaryIconLink>
                </div>
            }
        >
            <Head title={`${purchaseType} Purchases`} />
            <div className='mx-auto max-w-[96%] py-6'>
                <CustomDataTable
                    title={`${purchaseType} Purchases`}
                    data={purchases}
                    columns={columns}
